"""Build a final Query-Robust M32 asset from tokenized 64K JSONL contexts."""
...
def _capture_queries(
    ...
    *,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("A CUDA device is required for 64K calibration.")
    ...
    try:
        with torch.inference_mode():
            for sequence_index, context in enumerate(contexts):
                ...
                print(f"QR_LOCAL_SEQUENCE={sequence_index + 1}/{NUM_SEQUENCES}", flush=True)
    finally:
        active_positions[0] = None
        modeling_llama.apply_rotary_pos_emb = original_apply
        for layer, original_forward in zip(decoder.layers, original_forwards, strict=True):
    ...
    if any(labels.count(label) != expected_per_bucket for label in POSITION_WINDOWS):
        raise RuntimeError("The 64K calibration buckets are not balanced.")
    ...
    payload = {
        ...
        "meta": {
            ...
            "capture": "final Pile-only post-RoPE Query-Robust 64K calibration",
            "num_sequences": NUM_SEQUENCES,
            ...
            "packing": "tokenized Pile contexts with EOS to exact 65536 tokens",
        },
    }
...
def main() -> None:
    ...
    capture_path, capture_meta = _capture_queries(
        ...
        output_path=args.output_dir / "pile64k_q_capture.pt",
    )
    collected_path = args.output_dir / "pile64k_collected.pt"
    collect(
        [capture_path],
    ...
    asset_path = args.output_dir / "qr_vertices_m32_64k.pt"
    select_vertices(
        collected_path,
        asset_path,
    ...
    if not torch.equal(
        ...
        raise RuntimeError("The final 64K asset does not have 32 valid vertices everywhere.")
